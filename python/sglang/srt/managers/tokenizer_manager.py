...
class TokenizerManager(TokenizerCommunicatorMixin):
    ...
    def __init__(
        ...
        self.trace_file = "/tmp/sglang_traces/temporal_analysis.jsonl"
    ...
    def _send_one_request(
        ...
        prefix_hash = hashlib.md5(
            str(tokenized_obj.input_ids[:50]).encode()
        ).hexdigest()
        trace_data = {
            "req_id": tokenized_obj.rid,
            "arrival_ts": created_time,
            "prefix_hash": prefix_hash,
            "prefix_len": min(50, len(tokenized_obj.input_ids)),
        }
